...
tldr: 中心体异常是癌症的重要特征，但其空间复杂性使传统分析受限。本文推出CenSegNet，一种通用的深度学习框架，利用双分支架构和不确定性引导优化，在多种组织中实现了高分辨率、单细胞级别的中心体分割。该研究通过对大规模乳腺癌样本的分析，揭示了中心体异常与临床预后的关联，并提供了开源工具以促进相关研究。
source: biorxiv
selection_source: fresh_fetch
motivation: 旨在解决传统图像分析在处理异质组织中中心体异常的空间复杂性和表型异质性时的局限性。
method: 提出了一种名为CenSegNet的模块化深度学习框架，集成双分支架构与不确定性引导的细化技术，支持多种成像模态。
result: 在乳腺癌组织样本中成功量化了数值和结构性中心体异常，并发现结构性异常与肿瘤分级、基因组改变及患者生存率密切相关。
conclusion: CenSegNet作为一个可扩展且通用的平台，为系统性研究中心体在癌症及其他上皮疾病中的生物学功能提供了强有力的工具。
---

## 摘要
中心体异常 (CA) 是上皮性癌症的一个标志，但由于传统图像分析的局限性，其空间复杂性和表型异质性仍未得到很好的解决。我们提出了 CenSegNet（中心体分割网络），这是一个模块化的深度学习框架，用于跨多种组织类型对中心体和上皮结构进行高分辨率、上下文感知的分割。通过集成双分支架构和不确定性引导的细化，CenSegNet 在免疫荧光和免疫组化模态中均实现了最先进的性能和泛化能力，在准确性和形态保真度方面优于现有模型。应用于包含来自 127 名患者的 911 个乳腺癌样本核心的组织微阵列 (TMA)，CenSegNet 首次实现了单细胞分辨率下数值和结构 CA 的大规模、空间分辨定量。这些 CA 亚型在机制上是解耦的，表现出截然不同的空间分布、年龄依赖性动态，并与组织学肿瘤分级、激素受体状态、基因组改变和淋巴结受累相关。结构性 CA 水平还与总生存期相关，支持了空间分辨 CA 模式的临床相关性。肿瘤边缘不一致的 CA 谱与局部侵袭性和间质重塑有关。为了支持广泛采用和可重复性，CenSegNet 作为一个开源 Python 库发布。总之，我们的研究结果将 CenSegNet 确立为一个可扩展、通用的平台，用于完整组织中空间分辨的中心体表型分析，从而能够系统地剖析该细胞器的生物学及其在癌症和其他上皮疾病中的失调。
